globals [ max-wknights ]  ; altrimenti schioppa

; Sheep and wolves are both breeds of turtles
breed [ wknights wknight]
breed [ blackknights lui]
...
to setup
  clear-all
  ifelse netlogo-web? [ set max-wknights 100 ] [ set max-wknights 300 ]
  ...
  create-wknights 1  ; create the sheep, then initialize their variables
  [
  ...
  create-blackknights 1  ; create the wolves, then initialize their variables
  [
    set shape "chess knight"
    set color black
...
to go
  ...
  if count wKnights > max-wKnights [ user-message "ar cavaliere nero NUN JE DEVI CACà ER CAZZO!" stop ]
  ask wKnights [
    move
  ...
  ask blackknights [
    kill-wknights ; wolves eat a sheep on their patch
  ]
...
to move  ; turtle procedure
  let x 0
  let y 0
  ifelse xcor < 0 [set x xcor + 1][if xcor > 0 [set x xcor - 1]]
  ifelse ycor < 0 [set y ycor + 1][if ycor > 0 [set y ycor - 1]]
  if x > -1 and x < 1 [set x 0 ]
  if y > -1 and y < 1 [set y 0 ]
  setxy x y
end


to reproduce-wknights  ; sheep procedure
    hatch 3 [ rt random-float 360 fd 1
    setxy random-xcor random-ycor]   ; hatch an offspring and move it forward 1 step

end


to kill-wknights  ; wolf procedure
  let prey one-of wknights-here                    ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [set energy 0]
    ask prey [ death ]                            ; kill it, and...
  ]
end
...
"" ""
PENS
"cavalieri bianchi" 1.0 0 -4539718 true "" "plot count wknights"
"cavaliere nero" 1.0 0 -16580092 true "" "plot count blackknights"
